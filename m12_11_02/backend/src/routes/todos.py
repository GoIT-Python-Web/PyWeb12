...
async def get_todos(limit: int = Query(10, ge=10, le=500), offset: int = Query(0, ge=0, le=200),
    ...
    todos = await repository_todos.get_todos(limit, offset, db)
    return todos


@router.get("/{todo_id}", response_model=TodoResponse)
async def get_todo(todo_id: int = Path(ge=1), db: AsyncSession = Depends(get_db)):
    todo = await repository_todos.get_todo(todo_id, db)
    if todo is None:
        raise HTTPException(
...
async def delete_todo(todo_id: int = Path(ge=1), db: AsyncSession = Depends(get_db)):
    todo = await repository_todos.remove_todo(todo_id, db)
    if todo is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="NOT FOUND",
